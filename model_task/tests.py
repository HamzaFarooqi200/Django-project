...
class IpLogMiddlewareTestCase(TestCase):
    def setUp(self):
        ...
        self.middleware = IpLogMiddleware(
...
class AccessControlTestCase(TestCase):
    def setUp(self):
        ...
        self.user = CustomUser.objects.create_user(
            email="testuser78@gmail.com", password="password"
        )
    ...
    def test_unauthenticated_access(self):
        ...
        self.assertRedirects(
            response, reverse("login") + "?next=" + reverse("thanks_view")
        )
...
class RateLimitMiddlewareTestCase(TestCase):
    ...
    def test_rate_limit_enforcement(self):
        ...
        RateLimit.objects.filter(user=self.user).update(
            login_time=timezone.now() - timezone.timedelta(minutes=2), req_count=0
        )
