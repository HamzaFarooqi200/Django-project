...
patients_with_diagnosis = Patient.objects.filter(
    patientsrecords__diagonses__icontains="Flu"
)
doctors_with_patients = Doctor.objects.filter(
    doctors__in=patients_with_diagnosis
).distinct()
...
doctors = Doctor.objects.annotate(num_patients=Count("doctors")).filter(
    num_patients__gt=5
)
...
pat = Patient.objects.filter(
    doctor__in=Doctor.objects.filter(specialization="Heart")
).values_list("name", flat=True)

# - Find the nurses who have not been assigned any patients.

nurse = Nurse.objects.exclude(
    id__in=Patient.objects.values_list("nurse_id", flat=True)
).values_list("name", flat=True)
...
name = Patient.objects.filter(
    patientsrecords__diagonses__icontains="Heart"
).values_list("name", flat=True)

# - Find the doctors who have patients of a certain age group.

name = Doctor.objects.filter(doctors__age__gte=20, doctors__age__lte=40).values_list(
    "name", flat=True
)
...
name = Nurse.objects.filter(nurses__age__gte=20, nurses__age__lte=40).values_list(
    "name", flat=True
)
...
doc = Doctor.objects.filter(
    doctors__patientsrecords__prescription__icontains="Panadol"
).values_list("name", flat=True)
...
doc = Doctor.objects.filter(doctors__date_admitted__date=spec_date).values_list(
    "name", flat=True
)
...
nurse = Nurse.objects.filter(nurses__date_admitted=spec_date).values_list(
    "name", flat=True
)
...
pat = Patient.objects.filter(nurse__contact_number="031548525145").values_list(
    "name", flat=True
)
...
doc = Doctor.objects.annotate(avg_age=Avg("doctors__age")).values_list(
    "name", "avg_age"
)
...
pat = Patient.objects.filter(doctor__contact_number="031548525145").values_list(
    "name", flat=True
)
...
doc = (
    Doctor.objects.filter(specialization="Heart")
    .annotate(num_pat=Count("doctors"))
    .values_list("name", "num_pat")
)
...
pat = Patient.objects.filter(
    patientsrecords__diagonses__icontains="Diabetes", doctor__name="Dr. Smith"
).values_list("name", flat=True)
...
name = Doctor.objects.filter(
    doctors__age__gte=20,
    doctors__age__lte=40,
    doctors__patientsrecords__diagonses="Heart",
)
...
pat = Patient.objects.annotate(total_doc=Count("doctor")).values_list(
    "name", "total_doc"
)
...
pat = Patient.objects.annotate(total_nurse=Count("nurse")).values_list(
    "name", "total_nurse"
)
...
pat = Patient.objects.annotate(num_rec=Count("patientsrecords")).values_list(
    "name", "num_rec"
)

# - Select all patients with their diagnoses and prescriptions, if available.

pat = Patient.objects.select_related("patientsrecords").values(
    "name", "patientsrecords__diagonses", "patientsrecords__prescription"
)
...
pat = Patient.objects.annotate(num_records=Count("patientsrecords")).values(
    "name", "num_records"
)

# - Select all doctors with the count of patients they are associated with.

doc = Doctor.objects.annotate(num_patients=Count("doctors")).values(
    "name", "num_patients"
)
...
pat = Patient.objects.annotate(num_doc=Count("doctor", distinct=True)).values(
    "name", "num_doc"
)
